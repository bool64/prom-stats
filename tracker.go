...
func (t *Tracker) collect(ctx context.Context, isGauge bool, name string, value float64, labelsAndValues []string) {
	...
	if !ok {
		collect = t.createCollector(ctx, id, lb, name, isGauge)
	}
	t.mu.Unlock()
...
func (t *Tracker) createCollector(ctx context.Context, id identity, lb *labelBuffer, name string, isGauge bool) func(map[string]string, float64) {
...
func (t *Tracker) summary(
	...
	opts.Help = coalesce(opts.Help, "is created by "+callerFunc(5))
